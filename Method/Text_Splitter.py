...
def extract_oie_for_chunk(chunk_text: str, max_triples: int = 5, silent: bool = True) -> Optional[str]:
    ...
    try:
        ...
        # Trích xuất quan hệ OIE từ chunk
...
def save_raw_oie_data(oie_data: List[Dict], chunk_id: str, output_dir: str, method_name: str = "text_splitter") -> Optional[str]:
    """
    Lưu dữ liệu OIE thô vào một tệp TSV.

    Args:
        oie_data (List[Dict]): Danh sách các từ điển chứa dữ liệu OIE thô.
        chunk_id (str): ID của đoạn văn bản gốc.
        output_dir (str): Thư mục đầu ra để lưu tệp.
        method_name (str, optional): Tên phương thức được sử dụng để phân đoạn văn bản. Mặc định là "text_splitter".

    Returns:
        Optional[str]: Đường dẫn đến tệp đã lưu, hoặc None nếu không thể lưu.
    """
    try:
...
def merge_chunks_from_group(
    ...
    for chunk_id, chunk_text_w_oie, _, base_text in group:
        if current_merge_base_text and count_tokens_accurate(" ".join(current_merge_base_text + [base_text])) > max_tokens:
...
def process_merge_buffer(
    ...
    for i in range(len(buffer)):
        ...
        if base_token_count >= min_tokens:
            if merge_candidate_group:
                processed.extend(merge_chunks_from_group(merge_candidate_group, max_tokens, f"{context_id}_{i}"))
                merge_candidate_group = []
    ...
    if merge_candidate_group:
        processed.extend(merge_chunks_from_group(merge_candidate_group, max_tokens, f"{context_id}_end"))
            ...
            if include_oie and chunk_text_content.strip():
                try:
                    ...
                    raw_oie_relations = extract_relations_from_paragraph(
                        chunk_text_content, # Sửa lỗi: sử dụng chunk_text_content
                        silent=True
                    )
