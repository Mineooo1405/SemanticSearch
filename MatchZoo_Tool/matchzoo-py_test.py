...
# Tải GloVe embedding (Giảm chiều để tiết kiệm RAM)
...
# Giải phóng bộ nhớ RAM sau khi không cần glove_embedding nữa
del glove_embedding
gc.collect()
...
# Tạo DataLoader, chỉ cần dataset và callback
trainloader = mz.dataloader.DataLoader(
    dataset=trainset,
    stage='train',
    callback=padding_callback,
    device=device # Chỉ định device cho DataLoader
)
...
# Chuyển model sang device đã chọn (GPU/CPU)
model.to(device)
...
trainer = mz.trainers.Trainer(
    ...
    validate_interval=None, # Đánh giá sau mỗi epoch
    epochs=5,
    device=device, # Chỉ định device cho Trainer
    save_dir='matchlstm_model' # Chỉ định thư mục để lưu model
)

# Bắt đầu huấn luyện
trainer.run()

# --- 8. (Tùy chọn) Lưu model và preprocessor ---
print("Bắt đầu lưu model và preprocessor...")
trainer.save_model() # Sử dụng trainer để lưu model
preprocessor.save('matchlstm_model/preprocessor') # Lưu preprocessor vào cùng thư mục
